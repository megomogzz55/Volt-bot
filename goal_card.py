...
Same jersey colors as in the photo. High quality sharp edges."""
        ...
        # الطلب الصح مع تحديد إن المطلوب صورة
...
def create_goal_card(player_code, scorer_name, assist_name,
    ...
    if muse_bytes:
        ...
        print("⚠️ Gemini فشل — هنستخدم الصورة الحقيقية للاعب")
        # استخدم الصورة الحقيقية لو Gemini فشل
        final_bytes = image_bytes if image_bytes else None

    if not final_bytes:
        print("❌ مفيش صورة خالص")
        return None, make_goal_caption(scorer_name, assist_name, minute,
