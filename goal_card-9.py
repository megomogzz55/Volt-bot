...
def to_muse_style(image_bytes):
    ...
    بيجرب الحلول بالترتيب:
    1. Pollinations kontext (مجاني بدون key)
    2. Gemini (لو عنده quota)
    """

    # ══════════════════════════════════════════
    # الحل 1 — Pollinations kontext (بدون key)
    # ══════════════════════════════════════════
    try:
        import base64
        from urllib.parse import quote

        print("🎨 بنجرب Pollinations kontext...")

        # ارفع الصورة على imgbb أو استخدم base64 URL
        # Pollinations kontext بياخد image URL
        # هنحوّل الصورة لـ base64 data URL
        b64 = base64.b64encode(image_bytes).decode()
        image_url = f"data:image/png;base64,{b64}"

        prompt = "Convert to Muse State flat cartoon illustration. Flat vector art, bold black outlines, solid flat colors only, zero gradients, clean minimal cartoon face, white background, upper body portrait, same jersey colors."
